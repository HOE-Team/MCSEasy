# coding=utf-8
# Copyright (C) 2024-2025 HOE Team.
...
# 获取当前时间并格式化为文件名
log_filename = os.path.join(logs_dir, datetime.datetime.now().strftime("MCSEasy_log_%Y-%m-%d-%H-%M-%S.log"))
...
def check_java_in_path() -> str:
    """检查Java是否在环境变量中"""
    if shutil.which('java'):
        return ""
    else:
        return "未在系统PATH中找到Java可执行文件，服务器可能无法启动。"
...
def check_hardware():
    ...
    if total_memory_mb < 2048:  # 测试用更大的数值<----------
...
class VersionFetchThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.error_occurred.emit(f"获取版本列表失败: {e}")
    ...
    def fetch_versions(self):
        ...
        if self.server_type == "Paper":
            ...
            with urllib.request.urlopen(api_url) as response:
                ...
                versions = sorted(data.get("versions", []), key=version_key, reverse=True)
            ...
            with urllib.request.urlopen(api_url) as response:
                ...
                with urllib.request.urlopen(game_versions_url) as game_response:
                    game_versions = json.loads(game_response.read().decode())
                
                release_versions = [v['version'] for v in game_versions if v['stable']]
                snapshot_versions = [v['version'] for v in game_versions if not v['stable']]
                
                versions = list(dict.fromkeys(release_versions + snapshot_versions))
        
        elif self.server_type == "Forge":
            api_url = "https://bmclapi2.bangbang93.com/forge/minecraft"
            with urllib.request.urlopen(api_url) as response:
                data = json.loads(response.read().decode())
                ...
                versions = sorted(data, key=version_key, reverse=True)
...
class ServerDeployWindow(QMainWindow):
    ...
    def _run_initial_checks(self):
        ...
        # 检查 Java
        java_warning = check_java_in_path()
        if java_warning:
            warnings.append(java_warning)
            
        # 检查硬件
        warnings.extend(check_hardware())
        
        self._update_warning_bar(warnings)
    ...
    def _setup_ui(self):
        ...
        top_layout.addWidget(self.content_stacked_widget, stretch=2)
        ...
        top_layout.addWidget(self.right_widget, stretch=3)

        # 4. 底部警告条
        self.warning_bar_container = QWidget()
        self.warning_bar_layout = QVBoxLayout(self.warning_bar_container)
        self.warning_bar_layout.setContentsMargins(0, 0, 0, 0)
        self.warning_bar_container.hide() # 初始隐藏
        
        # 将顶部布局和警告条添加到主布局中
        main_layout.addLayout(top_layout)
        main_layout.addWidget(self.warning_bar_container)
    
    def _create_sidebar(self):
        """创建左侧侧边栏，包含主页、下载、设置、关于"""
        sidebar = QWidget()
        ...
        self.sidebar_list.addItem(QListWidgetItem("下载服务端"))
    ...
    def _create_content_stack(self):
        ...
        stacked_widget.addWidget(self._create_home_page()) # 0
        stacked_widget.addWidget(self._create_download_page()) # 1
        stacked_widget.addWidget(self._create_settings_page()) # 2
        stacked_widget.addWidget(self._create_about_page()) # 3
        # 其他功能页面不显示在侧边栏，但可通过按钮跳转
        stacked_widget.addWidget(self._create_memory_page()) # 4
        stacked_widget.addWidget(self._create_config_page()) # 5
    ...
    def _create_settings_page(self):
        ...
        self.stop_timeout_input.setFixedWidth(100)
    ...
    def _create_about_page(self):
        ...
        about_text.setHtml("""
            ...
            <p><a href="https://github.com/HOE-Team">HOE Team[↗]</a></p>
            <h4>开源协议：</h4>
    ...
    def _create_download_page(self):
        ...
        # 服务端类型选择
        group_layout.addWidget(QLabel("选择服务端类型:"))
        self.server_type_combo = QComboBox()
        self.server_type_combo.addItems(["Vanilla", "Fabric", "Forge", "Paper"])
        group_layout.addWidget(self.server_type_combo)

        # 版本选择
        group_layout.addWidget(QLabel("选择版本:"))
        self.version_combo = QComboBox()
        group_layout.addWidget(self.version_combo)

        # 刷新按钮和下载按钮
        refresh_btn = QPushButton("刷新版本列表")
        refresh_btn.setObjectName("pageActionButton")
        ...
        # 连接信号，当类型改变时，更新版本列表
        self.server_type_combo.currentTextChanged.connect(self._fetch_versions)
    ...
    def _fetch_versions(self):
        ...
        self.log_signal.emit(f"[MCSEasy-INFO] 正在获取 {server_type} 的版本列表...")
        self.version_combo.clear()
        self.version_combo.addItem("加载中...")
        self.version_combo.setEnabled(False)
        self.download_action_btn.setEnabled(False)

        if self.version_fetch_thread and self.version_fetch_thread.isRunning():
            self.version_fetch_thread.quit()
            self.version_fetch_thread.wait()

        self.version_fetch_thread = VersionFetchThread(server_type)
        self.version_fetch_thread.versions_fetched.connect(self._on_versions_fetched)
        self.version_fetch_thread.error_occurred.connect(self._on_fetch_error)
    ...
    def _on_versions_fetched(self, versions):
        ...
        if versions:
            ...
            self.log_signal.emit("[MCSEasy-INFO] 版本列表获取成功。")
        else:
            self.version_combo.addItem("无可用版本")
            self.log_signal.emit("[MCSEasy-WARN] 未获取到版本列表。")

    def _on_fetch_error(self, message):
        """处理版本获取错误"""
        self.version_combo.clear()
        self.version_combo.addItem("获取失败")
        self.log_signal.emit(f"[MCSEasy-ERROR] {message}")
        self.version_combo.setEnabled(True)
    ...
    def _download_server(self):
        ...
        try:
            ...
            if server_type == "Paper":
                # Paper API: /v2/projects/paper/versions/<version>/builds/<build>/downloads/paper-<version>-<build>.jar
                ...
                with urllib.request.urlopen(api_url) as response:
                    data = json.loads(response.read().decode())
                    version_info = next((item for item in data["versions"] if item["id"] == version), None)
                    if version_info:
                        version_url = version_info["url"]
                        with urllib.request.urlopen(version_url) as version_response:
                            version_data = json.loads(version_response.read().decode())
                            download_url = version_data["downloads"]["server"]["url"]
            
            if not download_url:
                QMessageBox.critical(self, "错误", "无法为所选版本找到下载链接。")
                return

            self.log_signal.emit(f"[MCSEasy-INFO] 正在下载 {server_type} {version}...")
            self.download_thread = DownloadThread(download_url, path)
            ...
            self.log_signal.emit(f"[MCSEasy-ERROR] 下载失败: {e}")
            self.download_action_btn.setEnabled(True)
    ...
    def _update_warning_bar(self, warnings: list):
        """更新警告条"""
        # 清空现有警告
        while self.warning_bar_layout.count():
            item = self.warning_bar_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()
        ...
        for msg in warnings:
            ...
            is_critical = "Java" in msg
            if is_critical:
                label.setStyleSheet("background-color: #e74c3c; color: white; padding: 3px; font-weight: bold; border-radius: 4px;")
            else:
